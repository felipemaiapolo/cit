...
def set1(it, theta, gamma, c, a, b, skew, m, n, p, loss, alpha, B):
    ...
    #Output
    return [reject_stfr, reject_resit, reject_gcm, reject_rbpt, reject_rbpt2] 

def set2(it, theta, gamma, c, a, b, skew, m, n, p, loss, alpha, B):
    ...
    #Output
    return [reject_crt, reject_cpt, reject_rbpt] 
...
def plot_type1(results, sett, skew, loss, alpha=.05, xlabel='x', ylabel='y', legend=True):
    ...
    if 'set1' in sett: names=['STFR','RESIT','GCM','RBPT', 'RBPT2'] 
    ...
    if 'set1' in sett: x = np.array(results_mean.loc[ind_row].gamma)
    ...
    for j in range(len(names)):
        ...
        plt.fill_between(x, results_mean[:,j]-results_std[:,j], results_mean[:,j]+results_std[:,j], color=colors[i], alpha=.2)
    
    if legend:
        if 'set1' in sett: plt.legend(bbox_to_anchor=(.625, .65), loc='upper left', ncol = 1, prop={'size': 9.25}, borderaxespad=.0, frameon=True, framealpha=.6)                
